...
with tab_contexto:
    ...
    if resultado_clima is None:
        st.info(t("contexto_info_elegir_sitio"))
    elif error_clima:
        ...
        if "meta" in resultado_clima:
            ...
            st.success(t(
                ...
                elevacion=f"{_meta['elevacion_m']:.0f}", media=f"{media_confirmada:.2f}",
            ))
        ...
        st.caption(t("contexto_caption_rugosidad_destino", z0=z0_actual))
...
with tab_config:
    ...
    for i, c in enumerate(st.session_state.clusters):
        with st.container():
            ...
            c["N"] = cc2.number_input(t("equipos_label_cantidad"), min_value=1, max_value=20, value=c["N"], step=1, key=f"n_{i}")
            c["altura_buje"] = cc3.number_input(
                t("equipos_label_altura_buje"), min_value=0.5, max_value=150.0,
                value=c["altura_buje"], step=0.5, key=f"h_{i}",
                help=t("equipos_help_altura_buje"),
            )
            if cc4.button("✕", key=f"del_{i}", help=t("equipos_help_quitar_cluster")) and len(st.session_state.clusters) > 1:
                st.session_state.clusters.pop(i)
                st.rerun()

            _specs = SPECS_TURBINAS.get(c["modelo"])
            _ruta_img = RUTA_IMAGEN.get(c["modelo"])
            with st.expander(t("equipos_expander_ficha_tecnica", modelo=NOMBRES_MODELO.get(c['modelo'], c['modelo']))):
                if not _specs:
                    ...
                    with col_specs:
                        st.caption(t(
                            ...
                            parte=_specs['numero_parte'], clase=_specs['clase_iec'],
                        ))
                        st.markdown(t(
                            "equipos_ficha_specs_markdown",
                            potencia_w=_specs['potencia_nominal_w'],
                            viento_ms=_specs['viento_potencia_nominal_ms'],
                            cutin_ms=_specs['velocidad_cutin_ms'],
                            supervivencia_ms=_specs['velocidad_supervivencia_ms'],
                            generador=_specs['tipo_generador'],
                            polos=_specs['polos_generador'],
                            voltaje=_specs['voltaje_salida'],
                            altura_total=_specs['altura_total_m'],
                            diametro_rotor=_specs['diametro_rotor_m'],
                            peso=_specs['peso_total_kg'],
                            vida_anos=_specs['vida_diseno_anos'],
                            cimentacion=_specs['cimentacion_requerida'],
                        ))
    ...
    with st.expander(t("equipos_expander_parametros_avanzados")):
        z0 = st.selectbox(
            t("equipos_label_z0"), options=[0.03, 0.1, 0.3, 1.0],
            format_func=lambda z: {0.03: t("equipos_z0_campo_abierto"), 0.1: t("equipos_z0_cultivos_bajos"),
                                    0.3: t("equipos_z0_suburbano"), 1.0: t("equipos_z0_urbano_denso")}[z],
        ...
        metodo_bouquet = st.radio(
            ...
            format_func=lambda m: t("equipos_bouquet_real") if m == "real"
            else t("equipos_bouquet_lineal"),
        )

    st.divider()

    if not st.session_state.get("sitio_activo"):
        st.warning(t("equipos_warning_elegir_sitio"))
...
with tab_resultados:
    st.caption(t("resultados_caption_intro", tab=t("tabs_financiero")))
    ...
    if st.session_state.get("calculo_listo"):
        ...
        if resultado_clima is None:
            st.error(
                t("resultados_error_sin_estacion", tab=t("tabs_clima")),
            )
        elif error:
            st.error(error)
        else:
            ...
            c3.metric(t("resultados_metric_correccion_densidad"),
                      t("resultados_metric_correccion_densidad_valor",
            ...
            st.markdown(t("resultados_subtitulo_detalle_cluster"))
            tabla = pd.DataFrame([{
                t("resultados_col_modelo"): NOMBRES_MODELO.get(r["modelo"], r["modelo"]), t("resultados_col_n"): r["N"],
                t("resultados_col_buje"): r["altura_buje"], t("resultados_col_kwh_anio"): round(r["kwh_anual"]),
            ...
            media_confirmada = resultado_clima["media"]
            with st.expander(t("resultados_expander_perfil_viento_titulo")):
                _r0 = resultados[0]
                ...
                st.write(t("resultados_texto_perfil_viento_intro"))
                _conclusion = (
                    t("resultados_texto_verificacion_concuerda")
                    if abs(_v_pot / _r0['v_hub_medio'] - 1) < 0.15
                    else t("resultados_texto_verificacion_discrepa")
                )
                st.write(
                    t(
                        "resultados_texto_verificacion_independiente",
                        z0=str(z0), terreno=_terreno_dst,
                        v_pot=f"{_v_pot:.2f}", altura_buje=f"{_r0['altura_buje']:.1f}",
                        v_hub_medio=f"{_r0['v_hub_medio']:.2f}", conclusion=_conclusion,
                    )
                )

            with st.expander(t("resultados_expander_horario_titulo")):
                ...
                st.write(
                    t(
                        ...
                        kwh_correcto=f"{cmp['kwh_anual_correcto']:.0f}",
                        v_media=f"{cmp['v_media']:.2f}",
            ...
            st.caption(t("resultados_caption_footer"))
        ...
        st.info(t("resultados_info_configurar_primero",
                   tab=t("tabs_config"), boton=t("equipos_boton_calcular")))
...
with tab_financiero:
    ...
    modulo_financiero_activo = st.toggle(
        t("financiero_toggle_modulo_activo"),
        value=True,
        key="fin_modulo_activo",
        help=t("financiero_toggle_modulo_activo_help"),
    )

    if not modulo_financiero_activo:
        st.info(t("financiero_info_modulo_desactivado"))
    elif not st.session_state.get("calculo_listo"):
        st.info(t("financiero_info_configurar_primero"))
    else:
        resultado_clima = st.session_state.sitio_activo
        error = None if resultado_clima is None else resultado_clima.get("error")

        if resultado_clima is None:
            st.error(t("financiero_error_sin_estacion"))
        elif error:
            st.error(error)
        else:
            df_clima = resultado_clima["df_clima"]
            ...
            st.markdown(f"**{t('financiero_titulo_tarifa_electrica')}**")
            _opciones_modo_tarifa = [
                "Tarifa plana (USD/kWh)",
                "Tarifa horaria real de Costa Rica (ARESEP)",
                "Tarifa comercial de Costa Rica (T-CO)",
            ]
            _labels_modo_tarifa = {
                "Tarifa plana (USD/kWh)": t("financiero_opcion_tarifa_plana"),
                "Tarifa horaria real de Costa Rica (ARESEP)": t("financiero_opcion_tarifa_horaria"),
                "Tarifa comercial de Costa Rica (T-CO)": t("financiero_opcion_tarifa_comercial"),
            }
            modo_tarifa = st.radio(
                t("financiero_radio_modo_tarifa"),
                _opciones_modo_tarifa,
                format_func=lambda _op: _labels_modo_tarifa[_op],
                horizontal=True,
                key="fin_modo_tarifa",
                help=t("financiero_radio_modo_tarifa_help"),
            )
            ...
            if modo_tarifa == "Tarifa plana (USD/kWh)":
                tarifa_kwh_USD = st.number_input(
                    t("financiero_number_tarifa_plana"), min_value=0.01, value=0.15, step=0.01,
                    format="%.2f", key="fin_tarifa_kwh")
            elif modo_tarifa == "Tarifa comercial de Costa Rica (T-CO)":
                col_c1, col_c2, col_c3 = st.columns(3)
                with col_c1:
                    proveedor_co = st.selectbox(
                        t("financiero_selectbox_proveedor"), ["CNFL", "ICE"], key="fin_co_proveedor",
                        help=t("financiero_help_proveedor"),
                    )
                with col_c2:
                    tramo_label = st.selectbox(
                        t("financiero_selectbox_consumo_mensual"),
                        [t("financiero_opcion_tramo_pequeno"), t("financiero_opcion_tramo_grande")],
                        key="fin_co_tramo",
                        help=t("financiero_help_consumo_mensual"),
                    )
                tramo_co = "pequeno" if tramo_label.startswith("≤") else "grande"
                with col_c3:
                    tipo_cambio_crc_usd = st.number_input(
                        t("financiero_number_tipo_cambio"), min_value=1.0, value=_tipo_cambio, step=1.0,
                ...
                if resultado_co:
                    st.caption(
                        t(
                            ...
                            precio=f"{resultado_co['precio_crc_kwh']:.2f}",
                ...
                with col_t1:
                    proveedor_tou = st.selectbox(
                        t("financiero_selectbox_proveedor"), ["CNFL", "ICE"], key="fin_tou_proveedor",
                        help=t("financiero_help_proveedor"),
                    )
                opciones_tarifa_tou = {
                ...
                with col_t2:
                    tarifa_tou = st.selectbox(
                        t("financiero_selectbox_tarifa"), opciones_tarifa_tou, key="fin_tou_tarifa",
                        help=t("financiero_help_tarifa"),
                    )
                with col_t3:
                    tipo_cambio_crc_usd = st.number_input(
                        t("financiero_number_tipo_cambio"), min_value=1.0, value=_tipo_cambio, step=1.0,
                ...
                if resultado_tou:
                    ...
                    st.caption(
                        t(
                            ...
                            tarifa=f"{resultado_tou['tarifa_efectiva_usd_kwh']:.4f}",
                            ahorro_crc=f"{resultado_tou['ahorro_anual_crc']:,.0f}",
                            ahorro_usd=f"{resultado_tou['ahorro_anual_usd']:,.0f}",
                        )
                    )
                    _col_periodo = t("financiero_col_periodo")
                    _col_kwh_anio = t("financiero_col_kwh_anio")
                    _col_precio_crc = t("financiero_col_precio_crc_kwh")
                    _col_valor_usd = t("financiero_col_valor_usd_anio")
                    _labels_periodo = {
                        "Punta": t("financiero_periodo_punta"),
                        "Valle": t("financiero_periodo_valle"),
                        "Nocturno": t("financiero_periodo_nocturno"),
                    }
                    tabla_periodos = pd.DataFrame([
                        {_col_periodo: _labels_periodo.get(periodo, periodo), _col_kwh_anio: v["kwh"],
                         _col_precio_crc: v["precio_crc_kwh"], _col_valor_usd: v["usd"]}
                        for periodo, v in resultado_tou["desglose_por_periodo"].items()
                    ])
                    st.dataframe(
                        tabla_periodos.style.format({
                            _col_kwh_anio: "{:,.0f}", _col_precio_crc: "{:,.2f}", _col_valor_usd: "${:,.0f}",
                        }),
                        hide_index=True,
                    )

            with st.expander(t("financiero_expander_parametros_avanzados")):
                col_a1, col_a2 = st.columns(2)
                with col_a1:
                    vida_util_anos = st.number_input(
                        t("financiero_number_vida_util"), min_value=1, value=40, step=1)
                with col_a2:
                    tasa_descuento_pct = st.number_input(
                        t("financiero_number_tasa_descuento"), min_value=0.0, value=8.0, step=0.5)
            ...
            st.divider()
            st.markdown(f"**{t('financiero_titulo_seleccion_equipo')}**")
            st.caption(t("financiero_caption_seleccion_equipo"))
            ...
            for _i, _c in enumerate(st.session_state.clusters):
                ...
                if not _articulos_disponibles:
                    st.caption(t("financiero_caption_sin_precio", modelo=_nombre_modelo))
                ...
                _c["articulo"] = st.selectbox(
                    t("financiero_selectbox_articulo", modelo=_nombre_modelo, n=str(int(_c['N']))),
                    options=_opciones_articulo,
                    index=_idx_articulo, key=f"articulo_fin_{_i}",
                    help=t("financiero_help_articulo"),
                )
                ...
                st.caption(
                    t(
                        ...
                        precio_unitario=f"{_precio_unitario:,.0f}",
                        n=str(int(_c['N'])),
                        total_cluster=f"{_precio_unitario * _c['N']:,.0f}",
                    )
                )
            st.metric(t("financiero_metric_precio_total_proyecto"), f"${_precio_total_proyecto:,.0f}")
            st.caption(
                t("financiero_caption_precio_total_detalle")
                + (" " + t("financiero_texto_falta_articulo") if _algun_modelo_sin_precio else "")
            )

            st.divider()
            st.markdown(f"**{t('financiero_titulo_costeo_real')}**")
            st.caption(t("financiero_caption_costeo_real"))
            col_c1, col_c2, col_c3 = st.columns(3)
            with col_c1:
                costo_equipos_usd = st.number_input(
                    t("financiero_number_costo_equipos"),
            ...
            with col_c2:
                precio_venta_usd = st.number_input(
                    t("financiero_number_precio_venta"),
                    min_value=0.0, value=0.0, step=100.0, format="%.2f",
            ...
            with col_c3:
                mantenimiento_anual_usd = st.number_input(
                    t("financiero_number_mantenimiento_anual"),
                    min_value=0.0, value=0.0, step=50.0, format="%.2f",
                    key="fin_mantenimiento_anual",
                    help=t("financiero_help_mantenimiento_anual"),
                )
            ...
            if costo_equipos_usd > 0 and precio_venta_usd > 0:
                ...
                st.caption(
                    t("financiero_caption_margen", margen=f"{margen_usd:,.2f}", pct=f"{margen_pct:.0f}")
                )
            ...
            if precio_venta_usd <= 0:
                ...
                st.info(
                    t("financiero_info_error_tarifa_horaria", opcion=t("financiero_opcion_tarifa_plana"))
                )
            elif modo_tarifa == "Tarifa comercial de Costa Rica (T-CO)" and resultado_co is None:
                st.info(
                    t("financiero_info_error_tarifa_comercial", opcion=t("financiero_opcion_tarifa_plana"))
                )
            else:
                if modo_tarifa == "Tarifa plana (USD/kWh)":
                ...
                b1.metric(t("financiero_metric_energia_anual"),
                          t("financiero_valor_kwh_anio", kwh=f"{kwh_anual_total:,.0f}"))
                b2.metric(t("financiero_metric_ahorro_anual"),
                          t("financiero_valor_monto_por_anio", monto=f"{fin['ahorro_anual_USD']:,.0f}"))
                b3.metric(t("financiero_metric_mantenimiento_anual"),
                          t("financiero_valor_monto_por_anio", monto=f"{fin['mantenimiento_anual_USD']:,.0f}"))

                st.markdown(f"**{t('financiero_titulo_retorno_inversion')}**")
                c1, c2, c3, c4 = st.columns(4)
                c1.metric(t("financiero_metric_capex"), f"${fin['capex']:,.0f}")
                c2.metric(
                    t("financiero_metric_payback"),
                    t("financiero_valor_anos", n=f"{fin['payback_years']:.1f}")
                    if fin["payback_years"] is not None else t("financiero_no_disponible"),
                )
                c3.metric(
                    t("financiero_metric_roi"),
                    f"{fin['roi_percentage']:.0f}%" if fin["roi_percentage"] is not None
                    else t("financiero_no_disponible"),
                )
                es_viable = bool(fin["roi_percentage"] and fin["roi_percentage"] > 0)
                c4.metric(
                    t("financiero_metric_viabilidad"),
                    t("financiero_valor_viable") if es_viable else t("financiero_valor_no_viable"),
                )
                ...
                if fin["npv_usd"] is not None:
                    st.caption(
                        t(
                            ...
                            anos=str(int(vida_util_anos)),
                            tasa=f"{tasa_descuento_pct:.1f}",
                            npv=f"{fin['npv_usd']:,.0f}",
                        )
                    )
                ...
                if fin["opex_anual_neto"] <= 0:
                    ...
                    st.caption(
                        t(
                            ...
                            na=t("financiero_no_disponible"),
                        )
                    )
                    st.info(t("financiero_info_opex_negativo_contexto"))
...
with tab_especificacion:
    st.caption(
        t("especificacion_caption_intro",
          tab_config=t("tabs_config"), tab_resultados=t("tabs_resultados"))
    )
    ...
    if not st.session_state.get("calculo_listo"):
        ...
        if resultado_clima is None:
            st.error(t("resultados_error_sin_estacion", tab=t("tabs_clima")))
        elif error:
            st.error(error)
            ...
            st.error(t("especificacion_error_sin_resultado", tab=t("tabs_resultados")))
        else:
            ...
            st.markdown(f"### {t('especificacion_titulo_datos_generales')}")
            ...
            g3.metric(t("especificacion_metric_energia_anual"),
                       t("especificacion_valor_energia_anual", kwh=f"{kwh_anual_total:,.0f}"))
            g4.metric(t("especificacion_metric_elevacion"), f"{elevacion_m:.0f} m")
            st.write(t("especificacion_arquitectura_electrica", voltaje=VOLTAJE_TURBINAS_V))

            st.divider()
            st.markdown(f"### {t('especificacion_titulo_turbinas')}")
            ...
            for _clave, _cantidad in _cantidad_por_modelo.items():
                ...
                with st.container(border=True):
                    ...
                    with col_specs:
                        st.markdown(t("especificacion_turbina_titulo_cantidad",
                                       nombre=_specs['nombre'], cantidad=_cantidad))
                        st.caption(t("especificacion_turbina_fabricante", parte=_specs['numero_parte']))
                        _filas_turbina = [
                            (t("especificacion_campo_potencia_nominal"), f"{_specs['potencia_nominal_w']} W"),
                            (t("especificacion_campo_velocidad_nominal"), f"{_specs['viento_potencia_nominal_ms']} m/s"),
                            (t("especificacion_campo_velocidad_cutin"), f"{_specs['velocidad_cutin_ms']} m/s"),
                            (t("especificacion_campo_velocidad_supervivencia"), f"{_specs['velocidad_supervivencia_ms']} m/s"),
                            (t("especificacion_campo_tipo_rotor"), _specs["tipo_rotor"]),
                            (t("especificacion_campo_tipo_generador"), _specs["tipo_generador"]),
                            (t("especificacion_campo_diametro_rotor"), f"{_specs['diametro_rotor_m']} m"),
                            (t("especificacion_campo_altura_pala"), f"{_specs['altura_pala_m']} m"),
                            (t("especificacion_campo_peso"), f"{_specs['peso_total_kg']} kg"),
                            (t("especificacion_campo_cimentacion"), _specs["cimentacion_requerida"]),
                        ]
                        st.dataframe(
                            pd.DataFrame([{t("especificacion_col_campo"): f, t("especificacion_col_valor"): v}
                                          for f, v in _filas_turbina]),
            ...
            st.divider()
            st.caption(t("especificacion_caption_fuente_datos"))

            st.divider()
            st.markdown(f"### {t('especificacion_titulo_informe_ejecutivo')}")
            ...
            # de la app.
            if st.button(t("especificacion_boton_generar_informe")):
                try:
                    with st.spinner(t("especificacion_spinner_armando_informe")):
                        ...
                        if "meta" in resultado_clima:
                            ...
                            _fuente_texto = t(
                                ...
                                estacion=_meta['estacion'], pais=_meta['pais'], wmo=_meta['wmo'],
                                lat=f"{_meta['lat']:.4f}", lon=f"{_meta['lon']:.4f}",
                                elevacion=f"{_meta['elevacion_m']:.0f}", media=f"{_media_confirmada:.2f}",
                            )
                        else:
                            _fuente_texto = t(
                                "especificacion_pdf_fuente_media_sitio", media=f"{_media_confirmada:.2f}"
                            )
                    ...
                    st.error(t("especificacion_error_generar_informe", error=str(e)))

            if st.session_state.get("informe_ejecutivo_pdf"):
                if st.session_state.get("informe_ejecutivo_sin_financiero"):
                    st.caption(t("especificacion_caption_informe_sin_financiero", tab=t("tabs_financiero")))
                st.download_button(
                    t("especificacion_boton_descargar_informe"),
                    data=st.session_state["informe_ejecutivo_pdf"],
                    file_name=f"{t('especificacion_pdf_nombre_archivo_base')}_{date.today().isoformat()}.pdf",
